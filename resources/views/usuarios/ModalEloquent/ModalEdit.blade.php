...
<div class="modal fade" id="editUser{{ $user->id }}" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form method="POST" action="{{ route('user.EditUser', $user->id) }}" class="nav-link {{ Request::is('user*') ? 'active' : '' }}">
        @method('PUT')
        @csrf
              ...
              <div class="modal-body" id="cont_modal">
                  ...
                  <div class="form-group">
                    ...
                    <option type="text" value="{{ $user->profession->id}}">{{$user->profession->title}}</option>
                    </select>
                </div>
