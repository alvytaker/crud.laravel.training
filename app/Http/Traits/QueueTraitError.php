...
trait QueueTraitError {

    public function emailusersfails(){
        
        date_default_timezone_set("America/Santiago");
        $fechactual = date("Y-m-d");

        $user = DB::table('user')->select('user.id AS id_usuario','name AS nombre_usuario','email AS email_usuario','title AS nombre_profesion')
        ->join('profession','user.profession_id','=','profession.id')
        ->where("created_at","<=","$fechactual")->get();
       ...
       foreach($user as $us){
        ...
        $utilidades->reg_log("emailusersfails()", "QueueController", $fechactual, "Enviando", "Envio de correos");
